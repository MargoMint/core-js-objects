...
function sellTickets(queue) {
  ...
  queue.forEach((bill) => {
    switch (bill) {
      case 25:
        numberOfBillsFromSeller[25] += 1;
        break;
      case 50:
        if (numberOfBillsFromSeller[25] > 0) {
          numberOfBillsFromSeller[25] -= 1;
          numberOfBillsFromSeller[50] += 1;
        } else {
          canSell = false;
        }
        break;
      case 100:
        if (
          numberOfBillsFromSeller[25] > 0 &&
          numberOfBillsFromSeller[50] > 0
        ) {
          numberOfBillsFromSeller[50] -= 1;
          numberOfBillsFromSeller[25] -= 1;
        } else if (numberOfBillsFromSeller[25] >= 3) {
          numberOfBillsFromSeller[25] -= 3;
        } else {
          canSell = false;
        }
        break;
...
class CssSelector {
  ...
  combine(selector1, combinator, selector2) {
    this.parts.element = `${selector1.stringify()} ${combinator} ${selector2.stringify()}`;
    return this;
  }
  ...
  checkOrder(part) {
    ...
    if (this.order.includes(part)) return;

    const lastPart = this.order[this.order.length - 1];
    if (
      lastPart &&
      correctOrder.indexOf(part) < correctOrder.indexOf(lastPart)
    ) {
      throw new Error(
